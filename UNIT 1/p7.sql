...
then
      if p >= 40 and p < 50 then update mid1 set total=tot,per=p,result='Pass',Grade='D' where RLNO=xrlno;
elsif 
   p >= 50 and p < 60 then

   update mid1 set total=tot,per=p,result='Pass',Grade='C' where RLNO=xrlno;
elsif
     p >= 60 and p < 70 then update mid1 set total=tot,per=p,result='Pass',Grade='B' where RLNO=xrlno;
elsif 
    p >= 70 and p <= 100 
then
